...
//   // ── Render ─────────────────────────────────────────────────────────────
//   return (
//     <form onSubmit={handleSubmit} className="space-y-12 sm:space-y-10" noValidate>
...
function deepClone(p) {
  ...
  return {
    ...
    colorTones: (p.colorTones || []).map(t => ({ ...t })),
    chemicalComposition: (p.chemicalComposition || []).map(r => ({ ...r })),
    imagePreview: p.image ? `${BASE_URL}/uploads/${p.image}` : "",
    image: null,
  };
}
...
export default function ProductForm({ initial, categories, onSubmit, onCancel }) {
  ...
  function handleSubmit(e) {
    ...
    if (!form.imagePreview) fd.append("removeImage", "true");
    onSubmit(fd);
  }
